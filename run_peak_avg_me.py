...
def write_results(dataset, peak_avg, physio_select, 
	...
	write_nifti(peak_avg, analysis_str, params)


def run_peak_average(dataset, m_param, physio, l_window, r_window, 
					 min_peak_thres, peak_distance, out_dir=None, 
					 n_samples=100):
	# Load data
	func_data, physio_sig, params = load_data(
		dataset, physio=physio, multiecho=m_param
	) 
	...
	# Write out results
	write_results(dataset, peak_avg, physio, params, out_dir)
...
if __name__ == '__main__':
	...
	parser.add_argument('-p', '--physio',
						help='<Required> select physio',
						required=True,
						choices=['PPG_HR', 'PPG_PEAK_AMP', 'PPG_LOW', 'RSP_RVT'],
						default=None,
						type=str)
	...
	parser.add_argument('-dist', '--peak_distance',
	                    help='minimum distance between peaks (in volumes)', 
	                    required=False,
	                    default=10,
	                    type=float)
	parser.add_argument('-m', '--m_param',
                        help='Multiecho parameter - t2 or s0',
                        choices=['t2', 's0'], 
                        required=False,
                        default=None,
                        type=str)
	args_dict = vars(parser.parse_args())
	run_peak_average(args_dict['dataset'], args_dict['m_param'], args_dict['physio'],  
